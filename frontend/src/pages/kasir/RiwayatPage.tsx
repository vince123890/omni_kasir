...
import { Table, Card, Typography, Space, Input, Tag, Statistic, Row, Col, Spin } from 'antd'
import { SearchOutlined, HistoryOutlined } from '@ant-design/icons'
import { kasirTransactionApi } from '../../api'
...
export default function RiwayatPage() {
  ...
  const load = useCallback(() => {
    setLoading(true)
    kasirTransactionApi.list({ page, limit: 20, search: search || undefined, date: dateFilter || undefined })
      .then(res => { setTxList(res.data); setTotal(res.meta?.total ?? 0) })
      .catch(() => {})
  ...
  const expandedRowRender = (record: Transaction) => {
    const cols = [
      ...
      { title: 'Harga', dataIndex: 'price', key: 'price', render: (v: number) => `Rp ${v.toLocaleString()}` },
      { title: 'Subtotal', dataIndex: 'subtotal', key: 'sub', render: (v: number) => <Text strong>Rp {v.toLocaleString()}</Text> },
    ]
    return (
  ...
  const columns = [
    ...
    { title: 'Waktu', dataIndex: 'createdAt', render: (v: string) => new Date(v).toLocaleString('id', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: 'short' }) },
  ...
  return (
    <Space direction="vertical" size={24} style={{ width: '100%', padding: 24 }}>
      <div>
        ...
        <Text type="secondary">Transaksi yang kamu proses</Text>
      </div>

      <Row gutter={[16, 16]}>
        <Col xs={12} sm={6}>
          <Card bordered={false} style={{ borderRadius: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <Statistic title="Total Transaksi" value={total} prefix={<HistoryOutlined />} valueStyle={{ color: '#4f46e5' }} />
          </Card>
        </Col>
        <Col xs={12} sm={6}>
          <Card bordered={false} style={{ borderRadius: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <Statistic title="Omzet" value={`Rp ${totalOmzet.toLocaleString('id-ID')}`} valueStyle={{ color: '#059669', fontSize: 18 }} />
          </Card>
        </Col>
      </Row>
      ...
      <Card bordered={false} style={{ borderRadius: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <Space style={{ marginBottom: 16, flexWrap: 'wrap' }}>
          ...
          <input type="date" value={dateFilter} onChange={e => { setDateFilter(e.target.value); setPage(1) }}
            style={{ padding: '4px 11px', border: '1px solid #d9d9d9', borderRadius: 6, fontSize: 14, height: 32 }} />
          {(search || dateFilter) && (
            <a style={{ color: '#dc2626', cursor: 'pointer', fontSize: 13 }} onClick={() => { setSearch(''); setDateFilter(''); setPage(1) }}>Reset</a>
          )}
        </Space>
        <Spin spinning={loading}>
          <Table columns={columns} dataSource={txList.map(t => ({ ...t, key: t.id }))} expandable={{ expandedRowRender }}
            pagination={{ current: page, pageSize: 20, total, onChange: setPage, showSizeChanger: false }}
            size="middle" locale={{ emptyText: 'Tidak ada transaksi' }} />
        </Spin>
      </Card>
    </Space>
  )
